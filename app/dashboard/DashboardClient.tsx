...
interface DashboardProps {
  ...
  children?: React.ReactNode; // TAMBAHKAN INI AGAR BISA MENERIMA KONTEN QUIZ
}
...
export default function DashboardClient({ user, allData = [], drivers, initialPeriod, children }: DashboardProps) {
      ...
      <main className={`flex-1 flex flex-col transition-all duration-500 ease-in-out ${isSidebarOpen ? 'ml-72' : 'ml-0'}`}>
        <header className="bg-white border-b border-slate-200 h-20 flex items-center justify-between px-8 sticky top-0 z-30 shadow-sm">
          <div className="flex items-center gap-4">
             ...
             {/* DATE PICKER HANYA MUNCUL JIKA TIDAK ADA CHILDREN (DASHBOARD SHIPMENT) */}
             {!children && (
          ...
          <div className="flex items-center gap-3">
             ...
             {/* TOMBOL INPUT HANYA MUNCUL DI DASHBOARD SHIPMENT */}
             {!children && (
        ...
        <div className="p-8 space-y-8 overflow-y-auto">
          {/* LOGIKA ANAK: Jika ada children (Quiz), tampilkan. Jika tidak, tampilkan Stats & Tabel Shipment */}
          {children ? (
            children
          ) : (
            <>
