...
import { updateUser, deleteUser, UserPayload } from '@/app/profile-actions';
...
export default function AdminEditProfileModal({ isOpen, onClose, userData }: ModalProps) {
  ...
  useEffect(() => {
    if (userData) {
      setFormData({
        ...
        password: '' // Kosongkan password saat edit, isi hanya jika ingin ubah
      });
    }
  }, [userData, isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.message || 'Gagal update user');
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    setIsLoading(true);
    try {
        const res = await deleteUser(formData.user_id);
        if (res.success) {
            window.location.reload();
        } else {
  ...
  return (
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-slate-50 w-full max-w-3xl rounded-3xl shadow-2xl overflow-hidden pointer-events-auto flex flex-col max-h-[90vh] border border-slate-200">
          ...
          <div className="p-5 border-b border-slate-200 bg-white flex justify-between items-center">
            <h2 className="text-lg font-black text-slate-800 uppercase">EDIT USER: {userData?.nama_lengkap}</h2>
          ...
          <div className="p-6 overflow-y-auto">
            ...
            <form id="editProfileForm" onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
              
              {/* KOLOM KIRI */}
              <div className="space-y-4">
                <h3 className="text-sm font-black text-indigo-600 border-b pb-2 mb-3">DATA PRIBADI</h3>
                
                <div><label className={labelClass}>User ID (NIK/KTP) - Read Only</label><input type="text" readOnly className={`${inputClass} bg-slate-200 text-slate-500 cursor-not-allowed`} value={formData.user_id} /></div>
          ...
          <div className="p-5 border-t border-slate-200 bg-white flex justify-between gap-3">
             <button type="button" onClick={() => setShowDeleteConfirm(true)} className="p-3 bg-rose-100 text-rose-600 font-bold rounded-xl hover:bg-rose-200 transition"><Trash2 size={20} /></button>
             ...
             <div className="flex gap-3">
                ...
                <button type="submit" form="editProfileForm" disabled={isLoading} className="px-8 py-3 bg-indigo-600 text-white font-black rounded-xl hover:bg-indigo-700 transition shadow-lg disabled:opacity-50 flex items-center gap-2">{isLoading ? 'Menyimpan...' : <><Save size={18}/> Simpan Perubahan</>}</button>
             </div>
          </div>
    ...
    <SystemModal 
        ...
        title="HAPUS USER?"
        message="Data user ini akan dihapus permanen. Lanjutkan?"
        type="confirm" 
    />
    </>
  );
