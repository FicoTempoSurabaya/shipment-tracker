...
import { Save, Loader2, Image as ImageIcon, Plus, X } from 'lucide-react'
...
export default function EditQuizModal({
  ...
  useEffect(() => {
    if (question) {
      setFormData({
        ...
        type_id: question.type_id || 'multiple_choice',
        is_scored: question.is_scored !== false,
        question_image_url: question.question_image_url || '',
        answers: question.answers || [],
        selectedCategories: question.categories?.map((c: any) => c.category_id) || []
      })
    }
  }, [question])
  ...
  const fetchTypesAndCategories = async () => {
    try {
      ...
      if (typesRes.ok) {
        // Menggunakan mapping yang sama seperti di InputQuizModal
